// Planet - 행성 클래스 (악기 인스턴스 관리)
// 각 행성은 하나의 악기를 담당하며, SONA 매핑을 통해 속성을 사운드로 변환합니다.

import * as Tone from 'tone';
import type { InstrumentRole, PlanetPhysicalProperties, PatternParameters } from '../../types/audio';
...
export class Planet {
  ...
  private mapExp(normalized: number, min: number, max: number, exponent: number = 2): number {
  ...
  private curve(normalized: number, type: 'exp' | 'log' | 'sigmoid'): number {
    ...
    switch (type) {
      ...
      case 'log': return Math.sqrt(n);
      case 'sigmoid': return this.sigmoid(n);
      default: return n;
    }
  }
  ...
  // (Tri) 음색 중심 속성 매핑 (1→3)
  private mapColorToSound(colorHue: number): { wtIndex: number; toneTint: number; wavefoldAmount: number } {
  ...
  private mapBrightnessToSound(brightness: number): { cutoffHz: number; outGainDb: number; resonanceQ: number } {
    ...
    const resonanceQ = this.clamp(qBase * (cutoffHz > 1200 ? 1.0 : 0.6), 0.2, 0.7);
    
    return { cutoffHz, outGainDb, resonanceQ };
  }
  
  private mapDistanceToSound(distance: number): { reverbSend: number; delayBeats: number; reverbSize: number } {
    const distN = distance / 100;
    // 역할별 리버브 상한 적용
    const sendMax = this.role === 'PAD' ? 0.6 : this.role === 'BASS' ? 0.15 : 0.25;
  ...
  private mapTiltToSound(tilt: number): { pan: number; msBlend: number; stereoWidth: number } {
  ...
  private mapSpinToSound(spin: number): { tremHz: number; tremDepth: number; chorusDepth: number } {
    const spinN = spin / 100;
    const tremHz = this.mapExp(spinN, 0.5, 8.0, 2.0);
    // ARPEGGIO trem 상한 적용
    const tremHzLimited = this.role === 'ARPEGGIO' ? Math.min(tremHz, 6.0) : tremHz;
    const tremDepth = this.mapLin(spinN, 0.10, 0.40);
  ...
  private mapSizeToPitch(size: number): { pitchSemitones: number; widthSemi: number } {
    const sizeN = size / 100;
    const pitchSemitones = this.mapExp(sizeN, -7, 7, 2.5);
    const widthSemi = Math.round(this.mapLin(Math.pow(sizeN, 0.6), 5, 19));
    
    return { pitchSemitones, widthSemi };
  }
  
  private mapSpeedToPattern(speed: number): { rateSync: string; pulses: number } {
  ...
  private mapEccentricityToGroove(eccentricity: number): { swingPct: number; accentDb: number } {
  ...
  private mapElevationToPitch(elevation: number): { octave: number; filterTypeMorph: number } {
  ...
  private mapPhaseToPattern(phase: number): { rotation: number; accentGate: boolean[] } {
    ...
    accentPhases.forEach(accentPhase => {
      if (Math.abs(phaseN - accentPhase) < 0.05) { // 5% 허용 범위
  ...
  private updateInstrument(): void {
    ...
    const brightnessMapping = this.mapBrightnessToSound(this.properties.brightness);
    ...
    // Eccentricity → 스윙 + 액센트
    const eccentricityMapping = this.mapEccentricityToGroove(this.properties.eccentricity);
    console.log(`🎭 Eccentricity 매핑:`, eccentricityMapping);
    
    // Elevation → 옥타브 + 필터 타입 모핑
    const elevationMapping = this.mapElevationToPitch(this.properties.elevation);
  ...
  async startPattern(): Promise<void> {
    ...
    if (!audioEngine.isReady()) {
      throw new Error('AudioEngine이 초기화되지 않았습니다.');
    }
    
    // 속성을 패턴 파라미터로 변환
    const patternParams = this.calculatePatternParams();
    
    // 루프 생성
    this.loop = audioEngine.createPatternLoop(patternParams, (stepIdx, accent) => {
      if (!this.isPlaying || this.instrument.isDisposed()) {
        return;
      }
      
      try {
        // SONA 지침: Euclidean 패턴 기반 재생 조건 확인
        if (this.shouldPlayOnStep(stepIdx)) {
          // 역할별 노트 생성
          const note = this.generateNoteForStep(stepIdx);
          
          // 액센트 정보 활용 (Eccentricity 매핑 + Phase 액센트 게이트)
          const patternParams = this.calculatePatternParams();
          const generatedPattern = this.generateEuclideanPattern(patternParams);
          const isAccentStep = generatedPattern.accents[stepIdx % generatedPattern.accents.length] === 1;
          
          // 최종 액센트 결정 (기본 액센트 + 패턴 액센트)
          const finalAccent = accent || isAccentStep;
          
          // Eccentricity 매핑을 통한 velocity 계산
          const eccentricityMapping = this.mapEccentricityToGroove(this.properties.eccentricity);
          const baseVelocity = finalAccent ? 0.8 : 0.6;
          const accentBoost = finalAccent ? (eccentricityMapping.accentDb / 10) : 0; // dB를 velocity로 변환
          const velocity = Math.min(1.0, baseVelocity + accentBoost);
          
          // Distance 기반 노트 지속시간 조절 (향후 구현 예정)
          const baseDuration = this.getNoteDuration();
          
          // 항성의 키/스케일로 양자화
          const quantizedNote = this.quantizeNote(note);
          
          this.instrument.triggerAttackRelease(quantizedNote, baseDuration, undefined, velocity);
          
          console.log(`🎵 ${this.name} [${stepIdx}] ${quantizedNote} vel:${velocity.toFixed(2)} ${finalAccent ? 'ACCENT' : ''}`);
        }
      } catch (error) {
        console.error(`${this.name} 재생 오류:`, error);
      }
    });
    
    // 루프 시작
    audioEngine.startLoop(this.loop);
    this.isPlaying = true;
    
    console.log(`▶️ ${this.name} 패턴 시작`);
  }
  ...
  stopPattern(): void {
    ...
    const audioEngine = AudioEngine.instance;
    audioEngine.cleanupLoop(this.loop);
    
    this.loop = undefined;
    this.isPlaying = false;
    
    console.log(`⏹️ ${this.name} 패턴 정지`);
  }
  ...
  private calculatePatternParams(): PatternParameters {
    ...
    const adjustedPulses = Math.round(speedMapping.pulses * complexityMultiplier);
  ...
  private applyRoleGuardrails(params: PatternParameters): void {
    switch (this.role) {
      ...
      case 'ARPEGGIO':
        ...
        console.log(`🎹 ARPEGGIO 가드레일: pulses 6-16 (${params.pulses}), rotation 2배수 (${params.rotation})`);
        break;
        
      case 'PAD':
        // PAD: pulses 2-6, gate 0.70-0.95, reverb_size 0.4-0.9
        params.pulses = Math.max(2, Math.min(6, params.pulses));
        params.gateLen = Math.max(0.70, Math.min(0.95, params.gateLen));
        console.log(`🌫️ PAD 가드레일: pulses 2-6 (${params.pulses}), gate 0.70-0.95 (${params.gateLen})`);
        break;
  ...
  private generateNoteForStep(stepIdx: number): string {
    ...
    switch (this.role) {
      case 'BASS':
        ...
        rawMidi = center + scaleNotes[noteIndex % scaleNotes.length] + sizeMapping.pitchSemitones;
        // BASS 가드레일: 28..52 범위
      ...
      case 'MELODY':
        ...
        if ((this.properties.color % 100) < 70) {
          ...
          noteIndex = (stepIdx + Math.floor(this.properties.phase / 45)) % scaleNotes.length;
        }
        rawMidi = center + scaleNotes[noteIndex % scaleNotes.length] + sizeMapping.pitchSemitones;
    ...
    // 디버그 로그
    console.log(`🎵 ${this.role} 노트 생성: center=${center}, width=${widthSemi}, raw=${rawMidi}, quantized=${quantizedMidi}, final=${finalNote}`);
    
    return finalNote;
  }
  
  // 역할별 재생 조건 (SONA 지침: Euclidean 패턴 기반으로 변경)
  private shouldPlayOnStep(stepIdx: number): boolean {
    // Euclidean 패턴 생성 및 활용
    const patternParams = this.calculatePatternParams();
    const generatedPattern = this.generateEuclideanPattern(patternParams);
    
    // 생성된 패턴에서 현재 스텝이 연주되어야 하는지 확인
    return generatedPattern.steps[stepIdx % generatedPattern.steps.length] === 1;
  }
  
  // Euclidean 패턴 생성 (SONA 매핑 시스템 활용)
  private generateEuclideanPattern(params: PatternParameters): { steps: number[], accents: number[] } {
    // Euclidean 리듬 알고리즘 구현
    const euclideanSteps = this.euclideanRhythm(params.pulses, params.steps);
    
    // Phase 기반 패턴 회전 적용
    const rotatedSteps = this.rotatePattern(euclideanSteps, params.rotation);
    
    // Phase 기반 액센트 패턴 생성
    const phaseMapping = this.mapPhaseToPattern(this.properties.phase);
    const accents = this.generateAccentPattern(rotatedSteps, phaseMapping.accentGate);
    
    return { steps: rotatedSteps, accents };
  }
  
  // Euclidean 리듬 알고리즘 (SONA 패턴 생성 핵심)
  private euclideanRhythm(pulses: number, steps: number): number[] {
  ...
  // 패턴 회전 (Phase 속성 활용)
  private rotatePattern(arr: number[], rotation: number): number[] {
    const n = arr.length;
    const r = ((rotation % n) + n) % n;
    return arr.slice(r).concat(arr.slice(0, r));
  }
  
  // 액센트 패턴 생성 (Phase 기반 quarter accent gate)
  private generateAccentPattern(steps: number[], accentGate: boolean[]): number[] {
    const accents = new Array(steps.length).fill(0);
    
    // 첫 번째 스텝은 항상 액센트 (기본 강박)
    if (steps[0] === 1) accents[0] = 1;
    
    // accentGate 배열에 따라 액센트 위치 결정
    accentGate.forEach((shouldAccent, index) => {
      if (shouldAccent && index < steps.length && steps[index] === 1) {
        accents[index] = 1;
      }
    });
