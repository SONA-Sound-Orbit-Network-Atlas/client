...
export type PropertyCategory = 'visual' | 'orbit' | 'audio' | 'pattern' | 'pitch';
...
// 기본값 생성
export function createDefaultProperties(overrides: Partial<PlanetProperties> = {}): PlanetProperties {
...
export function getPropertiesByCategory(category: PropertyCategory): PropertyDefinition[] {
  return Object.values(PLANET_PROPERTIES).filter(def => def.category === category);
}
