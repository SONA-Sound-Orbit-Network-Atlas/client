...
// 기본 항성 속성 생성
export function createDefaultStarProperties(overrides: Partial<StarProperties> = {}): StarProperties {
...
// 카테고리별 항성 속성 필터링
export function getStarPropertiesByCategory(category: PropertyCategory): PropertyDefinition[] {
  return Object.values(STAR_PROPERTIES).filter(def => def.category === category);
}

// 항성 속성을 오디오 매핑으로 변환
export function mapStarPropertiesToAudio(properties: StarProperties): Record<string, number> {
  ...
  Object.entries(properties).forEach(([key, value]) => {
    ...
    const normalizedValue = normalizeStarProperty(key, value || def.defaultValue);
...
export function starPropertiesToUI(properties: StarProperties): UIStarProperty[] {
